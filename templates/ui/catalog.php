...
$fieldLabels = [
    'sku' => 'SKU completo', 'ean' => 'EAN', 'supplier_id' => 'Fornitore Space', 'branch_suffix' => 'Filiale',
...
<section class="panel catalog-builder" id="publication-rules" aria-labelledby="publication-rules-title">
    ...
    <form class="integration-create__form catalog-filter-builder" action="/ui/catalog/publication-rules" method="post">
        ...
        <div class="catalog-filter-builder__sentence">
            ...
            <div class="field catalog-filter-builder__value">
                ...
                <input id="publication-value" name="match_value" required maxlength="500" placeholder="es. aec, A25, CD o EAN">
                <small>Per il fornitore usa il codice mostrato nell’anagrafica: per esempio <strong>aec</strong> per AEC ALLIANCE ENTERTAINMENT. <a href="/ui/suppliers">Consulta i fornitori Space</a>.</small>
            </div>
    ...
    <div class="inline-notice inline-notice--info catalog-rule-help">
        <div><strong>Come costruire la selezione</strong><span>EAN: usa “è uguale a” per un codice completo. Fornitore: usa il codice, ad esempio “aec”. Filiale: usa “A25”. Formato: usa valori come “CD” o “LP”. I confronti non distinguono maiuscole e minuscole.</span></div>
    </div>
    <?php endif; ?>
...
<details class="panel" id="new-pricing-rule">
    ...
    <form class="auth-form" action="/ui/catalog/pricing-rules" method="post" data-pricing-form>
        ...
        <div class="field"><label for="pricing-marketplace">Marketplace</label><select id="pricing-marketplace" name="marketplace_id"><option value="">Tutti quelli del catalogo</option><?php foreach (($marketplaces ?? []) as $marketplace): ?><?php if (in_array($marketplace['id'], $selectedCatalog['marketplace_ids'], true)): ?><option value="<?= $e((string) $marketplace['id']) ?>"><?= $e($marketplace['name']) ?> (<?= $e($marketplace['code']) ?>)</option><?php endif; ?><?php endforeach; ?></select><small>Lascia “Tutti” per una regola valida su ogni marketplace del catalogo.</small></div>
        <div class="field"><label for="pricing-match-field">Prodotti a cui applicarla</label><select id="pricing-match-field" name="match_field"><option value="">Tutti i prodotti selezionati</option><?php foreach ($fieldLabels as $value => $label): ?><option value="<?= $e($value) ?>"><?= $e($label) ?></option><?php endforeach; ?></select></div>
        <div class="field"><label for="pricing-match-operator">Condizione</label><select id="pricing-match-operator" name="match_operator"><option value="">Nessuna condizione</option><?php foreach ($operatorLabels as $value => $label): ?><option value="<?= $e($value) ?>"><?= $e(ucfirst($label)) ?></option><?php endforeach; ?></select></div>
        <div class="field"><label for="pricing-match-value">Valore</label><input id="pricing-match-value" name="match_value" maxlength="500" placeholder="es. aec, CD, A25 o EAN"><small>Per i fornitori usa il codice, ad esempio “aec”. Lascia vuoti i tre campi per applicare il prezzo a tutti i prodotti.</small></div>
...
<section class="panel data-panel" aria-labelledby="pricing-rules-title">
    ...
    <?php foreach ($pricingRules as $rule): ?>
        <tr><td><strong><?= $e($rule['name']) ?></strong><small><?= $e($rule['code']) ?></small></td><td><?= $e($scopeLabels[$rule['scope']] ?? $rule['scope']) ?></td><td><?= $e($rule['marketplace_code'] ?? '—') ?><small><?= ($rule['match_field'] ?? null) === null ? 'Tutti i prodotti' : $e(($fieldLabels[$rule['match_field']] ?? $rule['match_field']) . ' ' . ($operatorLabels[$rule['match_operator']] ?? $rule['match_operator']) . ' “' . $rule['match_value'] . '”') ?></small></td><td><strong><?= $e($formatAdjustment($rule)) ?></strong><small><?= $e($adjustmentLabels[$rule['adjustment_type']] ?? $rule['adjustment_type']) ?></small></td><td><?= $e($formatMoney($rule['minimum_price_minor'], $rule['currency'])) ?> / <?= $e($formatMoney($rule['maximum_price_minor'], $rule['currency'])) ?></td><td><?= $e((string) $rule['priority']) ?></td><td><?= $e((string) $rule['version']) ?></td><td><span class="status-badge status-badge--<?= $e($ruleTone($rule)) ?>"><?= $e($ruleStatus($rule)) ?></span></td></tr>
        ...
        <tr><td colspan="8"><details><summary>Modifica versione <?= $e((string) $rule['version']) ?></summary>
            ...
            <form class="auth-form" action="/ui/catalog/pricing-rules/<?= $e((string) $rule['id']) ?>" method="post" data-pricing-form>
                ...
                <div class="field"><label for="<?= $e($pricingFieldPrefix) ?>-marketplace">Marketplace</label><select id="<?= $e($pricingFieldPrefix) ?>-marketplace" name="marketplace_id"><option value="">Tutti quelli del catalogo</option><?php foreach (($marketplaces ?? []) as $marketplace): ?><?php if (in_array($marketplace['id'], $selectedCatalog['marketplace_ids'], true)): ?><option value="<?= $e((string) $marketplace['id']) ?>"<?= $selected($rule['marketplace_id'] === $marketplace['id']) ?>><?= $e($marketplace['name']) ?></option><?php endif; ?><?php endforeach; ?></select><small>La destinazione viene determinata automaticamente dal marketplace scelto.</small></div>
                <div class="field"><label for="<?= $e($pricingFieldPrefix) ?>-match-field">Prodotti a cui applicarla</label><select id="<?= $e($pricingFieldPrefix) ?>-match-field" name="match_field"><option value="">Tutti i prodotti selezionati</option><?php foreach ($fieldLabels as $value => $label): ?><option value="<?= $e($value) ?>"<?= $selected(($rule['match_field'] ?? null) === $value) ?>><?= $e($label) ?></option><?php endforeach; ?></select></div>
                <div class="field"><label for="<?= $e($pricingFieldPrefix) ?>-match-operator">Condizione</label><select id="<?= $e($pricingFieldPrefix) ?>-match-operator" name="match_operator"><option value="">Nessuna condizione</option><?php foreach ($operatorLabels as $value => $label): ?><option value="<?= $e($value) ?>"<?= $selected(($rule['match_operator'] ?? null) === $value) ?>><?= $e(ucfirst($label)) ?></option><?php endforeach; ?></select></div>
                <div class="field"><label for="<?= $e($pricingFieldPrefix) ?>-match-value">Valore</label><input id="<?= $e($pricingFieldPrefix) ?>-match-value" name="match_value" value="<?= $e($rule['match_value'] ?? '') ?>" maxlength="500" placeholder="es. aec, CD, A25 o EAN"></div><div class="field"><label for="<?= $e($pricingFieldPrefix) ?>-type">Come calcolare il prezzo</label><select id="<?= $e($pricingFieldPrefix) ?>-type" name="adjustment_type" data-pricing-type><?php foreach (['percentage' => 'Aumenta di una percentuale','fixed_amount' => 'Aggiungi un importo fisso','fixed_price' => 'Imposta un prezzo finale'] as $type => $label): ?><option value="<?= $e($type) ?>"<?= $selected($rule['adjustment_type'] === $type) ?>><?= $e($label) ?></option><?php endforeach; ?></select></div>
