...
final readonly class MarketplaceOrderObservation
{
    ...
    public static function fromEnvelope(MessageEnvelope $message): self
    {
        ...
        if ($normalizedCustomer['email'] !== null
            && filter_var($normalizedCustomer['email'], FILTER_VALIDATE_EMAIL) === false) {
        ...
        $normalizedShipping = [
            ...
            'phone' => self::nullableString($shipping, 'phone', 64) ?? $normalizedCustomer['phone'],
        ];
        ...
        foreach ($rawRows as $index => $rawRow) {
            ...
            $vat = self::requiredString($rawRow, 'vat_percent', 16);
            if (!preg_match('/^(?:0|[1-9][0-9]?|100)(?:\.[0-9]{1,2})?$/D', $vat)) {
            ...
            if (!str_contains($vat, '.')) {
                ...
                $taxRate = ((int) $whole * 100) + (int) str_pad($fraction, 2, '0');
            }
            $rows[] = [
