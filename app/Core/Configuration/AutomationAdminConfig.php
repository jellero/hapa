...
final readonly class AutomationAdminConfig
{
    public function __construct(
        ...
        if (!is_array($parts) || !in_array($parts['scheme'] ?? null, ['http', 'https'], true)
            ...
            throw new HapaRuntimeException('AUTOMATION_ADMIN_API_URL non valido.');
        }
        if ($production && ($parts['scheme'] ?? null) !== 'https') {
            throw new HapaRuntimeException('AUTOMATION_ADMIN_API_URL deve usare HTTPS in produzione.');
        }
        if (strlen($accessToken) < 32) {
            throw new HapaRuntimeException('AUTOMATION_ADMIN_API_TOKEN deve contenere almeno 32 caratteri.');
        }
        if ($timeoutSeconds <= 0 || $timeoutSeconds > 30) {
            throw new HapaRuntimeException('AUTOMATION_ADMIN_API_TIMEOUT non valido.');
        }
    }
}
