...
SQL);
    ...
    /** @param array<string, mixed> $result @throws JsonException */
    public function recordManualSupplierSync(int $id, array $result, UserIdentity $actor, string $correlationId): void
    {
        if (($result['status'] ?? null) !== 'completed' || !is_int($result['published'] ?? null)) {
            throw new HapaRuntimeException('Esito sincronizzazione fornitori Space non valido.');
        }
        $now = $this->clock->now()->format(DATE_ATOM);
        $encoded = json_encode([
            'status' => 'completed',
        ...
        $this->audit($id, 'integration.space_suppliers_synchronized_manually', $encoded, $actor, $correlationId, $now);
    }
