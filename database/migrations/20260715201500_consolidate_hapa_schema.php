...
final class ConsolidateHapaSchema extends AbstractMigration
{
...
ALTER TABLE shipments
    ADD CONSTRAINT shipments_status_check CHECK (
        status IN ('pending', 'requested', 'label_available', 'shipped', 'cancelled', 'error', 'manual_review')
    );
...
SQL);
    ...
    public function down(): void
    {
        ...
        $this->execute(<<<'SQL'
ALTER TABLE orders
    ALTER COLUMN shipping_address TYPE JSON USING shipping_address::json,
...
ALTER TABLE outbox_messages
    ...
    ALTER COLUMN updated_at TYPE TIMESTAMP USING updated_at AT TIME ZONE 'UTC';
ALTER TABLE external_deliveries
    ALTER COLUMN request_payload TYPE JSON USING request_payload::json,
    ALTER COLUMN response_payload TYPE JSON USING response_payload::json,
    ALTER COLUMN created_at TYPE TIMESTAMP USING created_at AT TIME ZONE 'UTC',
    ALTER COLUMN updated_at TYPE TIMESTAMP USING updated_at AT TIME ZONE 'UTC';
ALTER TABLE audit_logs
    ALTER COLUMN before_data TYPE JSON USING before_data::json,
    ALTER COLUMN after_data TYPE JSON USING after_data::json,
    ALTER COLUMN created_at TYPE TIMESTAMP USING created_at AT TIME ZONE 'UTC'
SQL);
    }
}
